...
getNewPoint( pos, targ )
{
	...
	if ( !isDefined( targ ) )
	{
		...
		foreach( player in level.players )
		{
			...
			if ( !level.teambased || player.team =="axis" )
...
harrierGetTargets()
{
	...
	for ( ;; )
	{
		...
		if ( isDefined( level.chopper ) && level.chopper.team != "allies" && isAlive( level.chopper ) )
		{	
...
isTarget( potentialTarget )
{
	...
	if ( level.teamBased && potentialTarget.pers["team"] == "allies" )//Dont kill humans ever
		return false;
...
checkForFriendlies( missileTarget, radiusSize )
{
	...
	for (i = 0; i < players.size; i++)
	{
		...
		if ( potentialCollateral.team != "allies" )
			continue;
