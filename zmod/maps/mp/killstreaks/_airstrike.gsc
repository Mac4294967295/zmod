...
{
	entall = maps\mp\gametypes\_weapons::getDamageableEnts(pos, radius, true);
	ents = [];
	foreach (e in entall)
		if (!e.isPlayer)//Skip players
			ents[ents.size] = e;
	//Add zombies
	foreach (player in level.players)
		if (player.team == "axis" &&  isalive(player) && player.sessionstate == "playing" )
			ents[ents.size] = get_damageable_player(player, get_damageable_player_pos(player));
...
callStrike_bomb( coord, owner, offset, showFx )
{
	...
	radiusArtilleryShellshock( bombPoint, 512, 8, 4, "allies" );
...
finishAirstrikeUsage( lifeId, location, directionYaw )
{
	...
	thread doAirstrike( lifeId, location, directionYaw, self, "allies" );
}
